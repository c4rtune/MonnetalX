...
def extract_markdown_links(text, repo_name):
    ...
    # ----------------------------------
    # 2. Remove Checklist section onward
    # ----------------------------------
    text = re.split(r'^\s*####\s+Checklist\s*$', text,
    ...
    # ----------------------------------
    # 4. Extract #12345 references
    # ----------------------------------
    issue_pattern = re.compile(r'(?<!\w)#(\d+)')
    nums = issue_pattern.findall(text)

    github_refs = [
        f"https://github.com/{repo_name}/pull/{n}"
        for n in nums
    ]

    # ----------------------------------
    # 5. Combine unique links
    # ----------------------------------
    results = []
    seen = set()

    for x in urls + github_refs:
        if x not in seen:
            seen.add(x)
            results.append(x)
